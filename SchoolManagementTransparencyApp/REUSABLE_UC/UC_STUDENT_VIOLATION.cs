...
namespace School_Management_Transparency.SchoolManagementTransparencyApp.UserControls
{
    public partial class UC_STUDENT_VIOLATION : UserControl
    {
        ...
        {
            courseDGV.DataSource = _courseDao.FetchCourseInstructorJoinData();
            if (courseDGV.Columns["ID"] != null) courseDGV.Columns["ID"].Visible = false;

        }

        private void PopulateTeacherDropdown()
        {
            // Call your unique DAO method
            DataTable teacherData = _courseDao.RetrieveTeacherLookupList();

            if (teacherData.Rows.Count > 0)
            {
                comboTeacherSelect.DataSource = teacherData;

                // WHAT THE USER SEES
                comboTeacherSelect.DisplayMember = "full_name";

                // WHAT THE DATABASE GETS (The Hidden ID)
                comboTeacherSelect.ValueMember = "teacher_id";
            }
        }
        ...
        {
            // This event fires the moment you click the checkbox, 
            // rather than waiting for you to click another row.
            if (studentLookupDGV.IsCurrentCellDirty)
            {
                studentLookupDGV.CommitEdit(DataGridViewDataErrorContexts.Commit);
            }
        ...
        private void showContainerPanelViolationType_Click(object sender, EventArgs e)
        {
            ...
            newViolationTypeContainer.Location = new Point(149, 23);
        ...
        private void violationTypeDGV_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                if (e.RowIndex >= 0)
                {
                    ...
                    // 2. Name (Alias is "Violation Name")
                    violationTypeNameTextBox.Text = row.Cells["Violation Name"].Value?.ToString() ?? "";

                    // 3. Fee (Alias is "Penalty Fee" - This was the likely mismatch)
                    violationTypeFeeTxtbox.Text = row.Cells["Penalty Fee"].Value?.ToString() ?? "";

                    // 4. Category (Alias is "Category")
                    violationTypeCategoryTxtbox.Text = row.Cells["Category"].Value?.ToString() ?? "";

                    // 5. Description (Alias is "Description")
                    violationTypeDescriptionTxtbox.Text = row.Cells["Description"].Value?.ToString() ?? "";
                }
            }
            catch (Exception ex)
            {
                // If it still fails, this will tell you exactly which Column Name is the problem
                MessageBox.Show("Mapping Error: " + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        {
            addCourseBtn.Enabled = false; // Disable to prevent double-click

            try
            {
                if (string.IsNullOrWhiteSpace(courseNameTxtbox.Text)) return;

                int chosenTeacherId = Convert.ToInt32(comboTeacherSelect.SelectedValue);

                if (_courseDao.InsertNewCourseRecord(courseNameTxtbox.Text.Trim(), chosenTeacherId))
                {
                    RefreshCourseGrid();
                    clearCourseBtn_Click(null, null);
                }
            }
            finally
            {
                addCourseBtn.Enabled = true; // Re-enable
                LoadCourseComboBox();
                LoadViolationTypes();
                RefreshGrid();
                LoadViolationTypeCRUDGrid();
        ...
        {
            // 1. Validation: Make sure an ID is selected
            if (string.IsNullOrEmpty(courseIdTxtbox.Text))
            {
                MessageBox.Show("Please select a course from the table first.", "Selection Required");
                return;
            }

            // 2. Gather values from the UI
            int currentCourseId = int.Parse(courseIdTxtbox.Text);
            string updatedName = courseNameTxtbox.Text.Trim();
            int updatedTeacherId = Convert.ToInt32(comboTeacherSelect.SelectedValue);

            // 3. EXECUTE ONLY ONCE
            // This calls your unique method name from CourseDao
            if (_courseDao.ModifyExistingCourseRecord(currentCourseId, updatedName, updatedTeacherId))
            {
                // Success feedback
                RefreshCourseGrid();
                clearCourseBtn_Click(null, null); // Reset the form after update
                LoadCourseComboBox();
                LoadViolationTypes();
                RefreshGrid();
                LoadViolationTypeCRUDGrid();
        ...
        {
            if (string.IsNullOrEmpty(courseIdTxtbox.Text)) return;

            if (MessageBox.Show("Delete this course?", "Confirm", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                // unique method call
                if (_courseDao.PermanentlyRemoveCourse(int.Parse(courseIdTxtbox.Text)))
                {
                    RefreshCourseGrid();
                    clearCourseBtn_Click(null, null);
                    LoadCourseComboBox();
        ...
        private void clearCourseBtn_Click(object sender, EventArgs e)
        {
            courseIdTxtbox.Clear();
            courseNameTxtbox.Clear();
            comboTeacherSelect.SelectedIndex = 0; // Reset to first teacher
            courseIdTxtbox.Clear();
            courseTeacherIdTxtbox.Clear();
        }

        private void backCourseBtn_Click(object sender, EventArgs e)
        {
            newCourseContainerPanel.Visible = false;
            backBtn.Enabled = true;
            addNewCourseBtn.Enabled= true;
            addNewCourseBtn.Enabled = true;
            showContainerPanelViolationType.Enabled = true;
            LoadCourseComboBox() ;
        ...
        {
            newCourseContainerPanel.Visible = true;
            newCourseContainerPanel.Location = new Point(149, 23);
            showContainerPanelViolationType.Enabled = false;
            backBtn.Enabled = false;
            addNewCourseBtn.Enabled = false;
            PopulateTeacherDropdown() ;
        ...
        private void courseDGV_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                // 1. Set the TextBoxes
                courseIdTxtbox.Text = row.Cells["ID"].Value.ToString();
                courseNameTxtbox.Text = row.Cells["Course Name"].Value.ToString();

                // 2. Set the Dropdown to match the "Instructor" column in your grid
                string instructorInGrid = row.Cells["Instructor"].Value.ToString();

                // This finds the index of the name and selects it
                int teacherIndex = comboTeacherSelect.FindStringExact(instructorInGrid);

                if (teacherIndex != -1)
                {
                    comboTeacherSelect.SelectedIndex = teacherIndex;
                }
            }
        }

        private void comboTeacherSelect_SelectedIndexChanged(object sender, EventArgs e)
        {
