...
//            String username = usernameLoginTxtbox.Text.Trim();
//            String password = passwordLoginTxtbox.Text.Trim();
...
//                if (role != null)
//                {
//                    // --- CRITICAL SESSION FIX HERE ---
//                    // Extract the true database User ID using the username string
//                    int loggedInId = userAccountController.GetUserIdByUsername(username);

//                    // Assign values to global memory state
//                    Util.UserSession.CurrentUserId = loggedInId;
//                    Util.UserSession.CurrentUsername = username;
//                    Util.UserSession.CurrentRole = role;
//                    // ---------------------------------
...
namespace School_Management_Transparency.SchoolManagementTransparencyApp.Winfroms
{
    public partial class LoginForm : Form
    {
        ...
        {
            // 1. Inputs Check
            if (string.IsNullOrWhiteSpace(usernameCreateTxtBox.Text) || string.IsNullOrWhiteSpace(passwordCreateTxtBox.Text) ||
                string.IsNullOrWhiteSpace(firstNameTextBox.Text) || string.IsNullOrWhiteSpace(lastNameTextBox.Text))
            {
                MessageBox.Show("Please check your input fields. Name and credentials cannot be left blank.", "Input Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            // 3. Executing Registration Transaction via the DB Layer Unified Methods
            bool registrationSuccess = _dao.CreateStudentWithAccount(
                firstNameTextBox.Text.Trim(),
                middleNameTextBox.Text.Trim(),
                lastNameTextBox.Text.Trim(),
            ...
            // 4. State Assignment and View Router Direction Update
            if (registrationSuccess)
            {
                MessageBox.Show("Registration Successful! Welcome to the system.", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);

                // Initialize standard user memory space fields
                int assignedId = userAccountController.GetUserIdByUsername(usernameCreateTxtBox.Text.Trim());
                Util.UserSession.CurrentUserId = assignedId;
                Util.UserSession.CurrentUsername = usernameCreateTxtBox.Text.Trim();
                Util.UserSession.CurrentRole = "STUDENT";
